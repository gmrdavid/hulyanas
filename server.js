...
app.get('/api/admin/stats', authenticateToken, isAdmin, async (req, res) => {
    try {
        ...
        const [[menuItems], [totalOrders], [revenue], [totalUsers]] = await Promise.all([
            ...
            conn.execute(`SELECT SUM(CASE WHEN LOWER(status) = 'delivered' THEN 1 ELSE 0 END) as delivered_today, SUM(CASE WHEN LOWER(status) = 'pending' THEN 1 ELSE 0 END) as pending_today, COUNT(*) as total_today FROM orders WHERE DATE(created_at) = CURDATE()`),
