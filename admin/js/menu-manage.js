let currentEditId = null;
        ...
        // Load menu items from database - UPDATED
        async function loadMenuItems() {
            try {
                // Use admin endpoint to show ALL items (including unavailable)
                const response = await fetch('/api/menu');
                if (!response.ok) throw new Error('Failed to fetch menu items');
                menuItems = await response.json();
                console.log(`📋 Loaded ${menuItems.length} menu items`); // Debug
        ...
        // Render table WITHOUT inline onclicks
        ...
        function handleImageError(img) {
            const defaultImg = '/images/default-menu.jpg?v=' + Date.now();
            img.src = defaultImg;
            img.onerror = null; // Prevent infinite loop
        }

        // Event delegation for table actions (CRITICAL FIX)
        function attachTableListeners() {
            const table = document.getElementById('menuTable');
            table.removeEventListener('click', handleTableClick); // Prevent duplicates
        ...
        async function deleteItem(id) {
            try {
                ...
                await loadMenuItems(); // Always refresh from server
